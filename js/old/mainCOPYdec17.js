...
var jerryGame = function() {
	...
	var rickScore = 0;
	var jerryScore = 0;
	var scoreDisplay = "<strong>Rick:  </strong>" +rickScore + "<br /><strong>Jerry:  </strong>" +jerryScore;
	
	var jerrysRandomizer = function () {
		...
		console.log("jerry Randomizer:  " + randomChoice);
		return randomChoice;
	};
	
	var jerryChooses = jerrysRandomizer();
	var roundWinner = "";
	
	var dialogGame = function () {
		var compare = function (rick, jerry) {
				if (jerryChooses === ricksChoice) {
					alert("You both chose " +ricksChoice + ". Play again.");
					roundWinner = "Tie";
				} else if (ricksChoice === "Rock") {
					if (jerryChooses === "Scissors") {
						rickScore++;
						roundWinner = "Rick";
					} else {jerryScore++; roundWinner = "Jerry";}
				} else if (ricksChoice === "Paper") {
					if (jerryChooses === "Rock") {
						rickScore++;
						roundWinner = "Rick";
					} else {jerryScore++; roundWinner = "Jerry"}
				} else if (ricksChoice === "Scissors") {
					if (jerryChooses === "Paper") {
						rickScore++;
						roundWinner = "Rick";
					} else {jerryScore++; roundWinner = "Jerry"}
				}
		...
		$(dialogMessage).dialog({
			...
			buttons: {
				"Rock": function () {
					ricksChoice = "Rock";
					$(this).dialog('close');
//					compare(ricksChoice, jerryChooses);
				},
				"Paper": function () {
					ricksChoice = "Paper";
					$(this).dialog('close');
...
//					compare(ricksChoice, jerryChooses);
		...
		console.log("Rick:  " + ricksChoice);
		console.log("Jerry:  " + jerryChooses);
		compare(ricksChoice, jerryChooses);
...
// combine the 2 functions and see if you still get child node error. Answer: had to use function to check how many children the element has
